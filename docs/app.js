...
// -- Now Playing --
function openNowPlaying(){nowPlayingOpen=true;$('#nowPlaying').style.display='block';document.body.style.overflow='hidden';applySettings();if(currentSong){updateNowPlayingPage();loadLyric(currentSong);updateNpFavBtn()}else{$('#npTitle').textContent='顶点音乐';$('#npArtistTag').textContent='歌手：未知';$('#npAlbumTag').textContent='专辑：未知';$('#npPlatformTag').textContent='网易云';$('#npQualityTag').textContent='标准';$('#npLyrics').innerHTML='<div class="lyrics-placeholder">暂无歌词</div>'}npShowControls();startNpIdleTimer()}
