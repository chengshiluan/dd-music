...
async function apiCall(p){if(p.platform==='netease'&&userState.github.loggedIn&&userState.github.id&&!p.github_id&&!p.cookie)p.github_id=userState.github.id;var r=await fetch(apiUrl(p));if(!r.ok)throw new Error('API '+r.status);return r.json()}
...
function parseLRC(t){if(!t)return[];var ls=t.split('\n'),r=[];ls.forEach(function(l){var m=l.trim().match(/^\[(\d{2}):(\d{2})\.(\d{2,3})\]\s*(.*)/);if(m){var time=parseInt(m[1])*60+parseInt(m[2])+parseInt(m[3])/(m[3].length===2?100:1000);if(m[4].trim())r.push({time:time,text:m[4].trim()})}});r.sort(function(a,b){return a.time-b.time});return r}
  ...
  // -- Lyric cache (localStorage, max 20, LRU) --
  var LYRIC_CACHE_MAX=20;
  function getLyricCache(){try{var s=localStorage.getItem('dd_music_lyric_cache');if(s)return JSON.parse(s)}catch{};return{}}
  function saveLyricCache(cache){try{var keys=Object.keys(cache);if(keys.length>LYRIC_CACHE_MAX){keys.sort(function(a,b){return(cache[a]._ts||0)-(cache[b]._ts||0)});var remove=keys.slice(0,keys.length-LYRIC_CACHE_MAX);remove.forEach(function(k){delete cache[k]})}localStorage.setItem('dd_music_lyric_cache',JSON.stringify(cache))}catch{}}
  function cachedLyricKey(tid,p){return p+':'+tid}

async function loadLyric(song){var tid=song.id||'',p=song.source||currentPlatform;if(p==='mine')p='netease';var cacheKey=cachedLyricKey(tid,p);var cache=getLyricCache();$('#npLyrics').innerHTML='<div class="lyrics-placeholder">加载歌词...</div>';try{if(cache[cacheKey]&&cache[cacheKey].lyric){cache[cacheKey]._ts=Date.now();saveLyricCache(cache);lyricLines=parseLRC(cache[cacheKey].lyric);if(!lyricLines.length){$('#npLyrics').innerHTML='<div class="lyrics-placeholder">暂无歌词</div>';return}$('#npLyrics').innerHTML=lyricLines.map(function(l,i){return'<div class="lyric-line" data-idx="'+i+'">'+escHtml(l.text)+'</div>'}).join('');return}var d=await apiCall({action:'lyric',platform:p,trackId:tid});lyricLines=parseLRC(d.lyric||'');cache[cacheKey]={lyric:d.lyric||'',_ts:Date.now()};saveLyricCache(cache);if(!lyricLines.length){$('#npLyrics').innerHTML='<div class="lyrics-placeholder">暂无歌词</div>';return}$('#npLyrics').innerHTML=lyricLines.map(function(l,i){return'<div class="lyric-line" data-idx="'+i+'">'+escHtml(l.text)+'</div>'}).join('')}catch(e){$('#npLyrics').innerHTML='<div class="lyrics-placeholder">歌词加载失败</div>'}}

function syncLyric(ct){if(!lyricLines.length||!nowPlayingOpen)return;var idx=-1;for(var i=lyricLines.length-1;i>=0;i--){if(ct>=lyricLines[i].time){idx=i;break}}if(idx===activeLyricIdx)return;activeLyricIdx=idx;var lines=$$('#npLyrics .lyric-line');lines.forEach(function(el,i){i===idx?el.classList.add('active'):el.classList.remove('active')});if(idx>=0&&lines[idx]){var c=$('#npLyrics');var lineEl=lines[idx];var target=lineEl.offsetTop-c.clientHeight/2+lineEl.clientHeight/2;if(target<0)target=0;c.scrollTo({top:target,behavior:'smooth'})}}
